...
const ReportsPage = () => {
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      pdf.text('Financial Report', margin, margin + 6);
      pdf.setFontSize(10);
      pdf.setTextColor(100, 100, 100);
      const periodLabel = view === 'monthly' ? monthLabel : `Annual ${year}`;
      pdf.text(periodLabel, margin, margin + 13);
      pdf.text(`Generated on ${new Date().toLocaleDateString()}`, margin, margin + 19);

      const chartStartY = margin + 24;
      const chartUsableH = pageH - chartStartY - margin;
      ...
      while (sourceY < imgH) {
        const availH = isFirstPage ? chartUsableH : usableH;
        const slicePx = Math.min(availH / scale, imgH - sourceY);
        ...
        ctx.fillRect(0, 0, imgW, slicePx);
        ctx.drawImage(canvas, 0, sourceY, imgW, slicePx, 0, 0, imgW, slicePx);

        const destY = isFirstPage ? chartStartY : margin;
        pdf.addImage(pageCanvas.toDataURL('image/png'), 'PNG', margin, destY, usableW, slicePx * scale);
      ...
      const filename = view === 'monthly'
        ? `report-${year}-${String(month).padStart(2, '0')}.pdf`
